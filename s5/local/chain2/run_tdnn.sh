...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr_lats.sh --nj $decode_nj --cmd "$train_cmd" ${lores_train_data_dir} \
...
if [ $stage -le 24 ]; then
  ...
  for data in $test_sets; do
    (
	  ...
	  --beam 20 --lattice-beam 12 \
          --nj $nspk --cmd "$decode_cmd"  --num-threads 4 \
          --online-ivector-dir exp/nnet3${nnet3_affix}/ivectors_${data}_hires \
          $tree_dir/graph_tgsmall data/${data}_hires ${dir}/decode_${data} || exit 1
