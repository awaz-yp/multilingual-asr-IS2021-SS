...
cleanup=true
speaker_only=true  # If true, utterances are only combined from the same speaker.
...
if ! python -c "x=float('$min_seg_len'); assert(x>0.0 and x<100000.0);" 2>/dev/null; then
...
#echo utils/data/internal/choose_utts_to_combine.py --min-duration=$min_seg_len \
#  --merge-within-speakers-only=$speaker_only \
#  $srcdir/spk2utt $srcdir/utt2dur $dir/utt2utts $dir/utt2spk $dir/utt2dur


python utils/data/internal/choose_utts_to_combine.py --min-duration=$min_seg_len \
  --merge-within-speakers-only=$speaker_only \
  $srcdir/spk2utt $srcdir/utt2dur $dir/utt2utts $dir/utt2spk $dir/utt2dur
