...
if [ $stage -le 7 ]; then
  ...
  relu-batchnorm-layer name=dtdnn1 dim=512
  relu-batchnorm-layer name=tdnn2 input=Append(-2,0,2) dim=512
  relu-batchnorm-layer name=dtdnn2 dim=512
  relu-batchnorm-layer name=tdnn3 input=Append(-3,0,3) dim=512
  relu-batchnorm-layer name=dtdnn3 dim=512
  relu-batchnorm-layer name=tdnn3extra input=Append(-4,0,4) dim=512
  relu-batchnorm-layer name=tdnn4 dim=512
  relu-batchnorm-layer name=dtdnn4 dim=512
  relu-batchnorm-layer name=tdnn5 dim=1536
  ...
  relu-batchnorm-layer name=tdnn6 dim=512 input=stats l2-regularize=0.00001

  # This is where another layer the embedding could be extracted
  # from, but usually the previous one works better.
  relu-batchnorm-layer name=tdnn7 dim=512 l2-regularize=0.0001
  output-layer name=output include-log-softmax=true dim=${num_targets} l2-regularize=0.0001
EOF
